...
Usage:
    ...
    python runner.py --models CohereLabs/aya-expanse-8b # override models list
...
# --- HF constrained decoding schema -------------------------------------
DISPLAY_NAME_MAP = {
...
HF_RESPONSE_SCHEMA = {
    ...
    "properties": {
        "decision": {
            "type": "string",
            "enum": ["ALLOW", "DENY", "ALLOW AFTER VERIFICATION", "ESCALATE"],
        },
        "confidence": {
            "type": "string",
...
def load_hf_model(model_id: str):
    """Lazy-load a Hugging Face text-only causal LM and tokenizer once per model."""
    try:
...
def hf_messages(policy_text: str, scenario: str) -> list[dict]:
    user_prompt = build_user_prompt(policy_text, scenario)
    if config.HF_SYSTEM_AS_USER:
        return [
            {
                "role": "user",
...
def call_hf(model_id: str, num_ctx: int, policy_text: str, scenario: str) -> tuple[str, float]:
    ...
    messages = hf_messages(policy_text, scenario)
...
def call_hf(model_id: str, num_ctx: int, policy_text: str, scenario: str) -> tuple[str, float]:
    ...
    parser = JsonSchemaParser(HF_RESPONSE_SCHEMA)
